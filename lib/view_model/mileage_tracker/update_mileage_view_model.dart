...
class UpdateMileageViewModel extends GetxController {
      ...
      Row(
        children: [
          Expanded(
            child: MyTextField(
              controller: startKm,
              title: 'Enter the Start Kilometre',
              showIcon: false,
              showPsw: false,
  ...
  setInitialValues(String id) async {
    DocumentSnapshot ds = await databaseConnection.getMileageById(id);
    Map<String, dynamic> map = ds.data() as Map<String, dynamic>;
    Mileage mileage = Mileage.fromJson(map);
    startKm.text = mileage.startKm!;
    endKm.text = mileage.endKm!;
    ttlLitres.text = mileage.ttlLitres.toString();
    price.text = mileage.price.toString();
    date.text = mileage.date!;
    update();
  }

  updateMileage(String? id) async {
    ...
    await databaseConnection.updateMileage(id!, mileage);
  }
}
